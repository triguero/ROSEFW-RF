...
import org.apache.mahout.classifier.feature_weighting.*;
...
public abstract class Builder {
  ...
  private final FWgenerator fw_algorithm;
  ...
  protected Builder(FWgenerator fw_algorithm, Path dataPath, Path datasetPath, String reduceType, Configuration conf, String cabecera) {
  ...
  protected FWgenerator getPGBuilder(){
    return fw_algorithm;  
  }
  ...
  public static FWgenerator getFWgeneratorBuilder(Configuration conf) {
    String string = conf.get("mahout.fc.fw_algorithm");
    if (string == null) {
      return null;
    }
  ...
  private static void setFWgeneratorBuilder(Configuration conf, FWgenerator fw_algorithm) {
    conf.set("mahout.fc.fw_algorithm", StringUtils.toString(fw_algorithm));
  }
  ...
  protected abstract double[] parseOutput(Job job) throws IOException;
  
  
  public double[] build() throws IOException, ClassNotFoundException, InterruptedException {
    ...
    */
    ...
    setFWgeneratorBuilder(conf, fw_algorithm);
    ...
    Job job = new Job(conf, "FW builder;"+fw_algorithm.FWmethod+" "+" "+dataPath.getName()+", "+this.reducePhase);
    
    log.debug("FW: Configuring the job...");
    configureJob(job);
    
    log.debug("FW: Running the job...");
    if (!runJob(job)) {
      log.error("FW: Job failed!");
      return null;
    }
    
   
    if (isOutput(conf)) {
      log.debug("FW: Parsing the output...; converting to RS");
      double[] resultingSet = parseOutput(job);

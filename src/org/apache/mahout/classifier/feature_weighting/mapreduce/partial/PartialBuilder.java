...
import org.apache.mahout.classifier.feature_weighting.mapreduce.*;
...
public class PartialBuilder extends Builder {
  ...
  public PartialBuilder(FWgenerator fw_algorithm, Path dataPath, Path datasetPath, String reduceType, String cabecera) {
    this(fw_algorithm, dataPath, datasetPath, reduceType, new Configuration(), cabecera);
  }
  
  public PartialBuilder(FWgenerator fw_algorithm,
                        Path dataPath,
                        Path datasetPath, String reduceType,
                        Configuration conf, String cabecera) {
    super(fw_algorithm, dataPath, datasetPath, reduceType, conf, cabecera);
  }
  ...
  protected void configureJob(Job job) throws IOException {
    ...
    job.setMapperClass(FWMapper.class);
  ...
  @Override
  protected double[] parseOutput(Job job) throws IOException {
    Configuration conf = job.getConfiguration();
  ...
  protected double[] processOutput(JobContext job,
  ...
  //  System.out.println("Outfiles size= "+outfiles.length);
   ...
   //read the output (un solo fichero por ser reduce)
	...
	return value.getSelectedFeatures();

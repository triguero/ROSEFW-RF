...
public class FeatureSeparator {
  ...
  public FeatureSeparator(Path inputPath, Path datasetPath, Path preprocessedPath, Path outputPath, Configuration conf, String cabecera, double WeightThreshold) {
  ...
  private static void setWeight(Configuration conf, Double weight) {
	    conf.set("mahout.fc.Double", StringUtils.toString(weight));
}
  ...
  public static Double getWeight(Configuration conf){
	    String string = conf.get("mahout.fc.Double");
	    if (string == null) {
	      return null;
	    }
  ...
  public void run() throws IOException, ClassNotFoundException, InterruptedException {
    ...
    setWeight(conf, this.WeightThreshold);
  ...
  public static class ClassifierMapper extends Mapper<LongWritable, Text, DoubleWritable, Text> {
   ...
   // private final Random rng = RandomUtils.getRandom();
    ...
    protected void setup(Context context) throws IOException, InterruptedException {
      ...
      weight=FeatureSeparator.getWeight(conf);
    //  log.info("cabecera: "+header);
      ...
      features= MapredOutput.load(conf, new Path(files[1].getPath()));

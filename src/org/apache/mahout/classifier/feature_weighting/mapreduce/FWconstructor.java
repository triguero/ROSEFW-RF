 ...
 */
public class FWconstructor extends Configured implements Tool{
  
  private static final Logger log = LoggerFactory.getLogger(FWconstructor.class);
  ...
  // TODO Auto-generated method stub
    ...
    Option preprocessedOpt = obuilder.withLongName("feature_weighting").withShortName("fw").withRequired(true).withArgument(
	        abuilder.withName("path").withMinimum(1).withMaximum(1).create()).
	        withDescription("Feature weights path").create();
    
    Option weight = obuilder.withLongName("weight threshold").withShortName("w").withRequired(true).withArgument(
	        abuilder.withName("path").withMinimum(1).withMaximum(1).create()).
	        withDescription("Weight threshold to select features").create();
	...
	Group group = gbuilder.withName("Options").withOption(inputOpt).withOption(testOpt).withOption(header).withOption(preprocessedOpt).withOption(weight).withOption(outputOpt).withOption(helpOpt).create(); 
	...
	try {
      ...
      this.WeightThreshold=Double.valueOf(cmdLine.getValue(weight).toString());
  ...
  private void testPreprocessedSet() throws IOException, ClassNotFoundException, InterruptedException {
   ...
   // PrototypeSet RS=PGUtils.readPrototypeSet(preprocessedPath.toString());
    ...
    FeatureSeparator fsSeparator = new FeatureSeparator(dataPath, datasetPath, fsPath, outputPath, getConf(), cabecera.getHeader(), WeightThreshold);
 ...
 */
  ...
  public static void main(String[] args) throws Exception {
    ToolRunner.run(new Configuration(), new FWconstructor(), args);
  }
}
